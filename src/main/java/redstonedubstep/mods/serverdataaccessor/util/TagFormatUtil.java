...
public class TagFormatUtil {
	public static ListTag formatResourceEntriesToKeys(ListTag listTag) {
		if (!listTag.isEmpty() && listTag.getFirst() instanceof CompoundTag) { //When the id tag gets referenced directly, only show the resource keys as string tags to truncate all the {} and resource ids, since these are not worth showing
			ListTag onlyKeyList = new ListTag();

			listTag.forEach(tag -> ((CompoundTag) tag).getString("K").ifPresent(s -> onlyKeyList.add(StringTag.valueOf(s))));
			return onlyKeyList;
		}
	...
	public static void removeNestedCollectionTags(CompoundTag tag) {
		if (!tag.isEmpty()) {
			List<Pair<String, Integer>> nestedTags = tag.keySet().stream().filter(s -> tag.get(s) instanceof CollectionTag || tag.get(s) instanceof CompoundTag).map(s -> Pair.of(s, TagFormatUtil.getTagSize(tag.get(s)))).toList();
	...
	public static void splitTagToPage(Tag tag, int currentPage, int entriesPerPage) {
		...
		if (tag instanceof CollectionTag collectionTag)
			...
			compoundTag.keySet().forEach(s -> tagList.add(Pair.of(s, compoundTag.get(s))));
	...
	public static void clearCollectionTag(Tag tag) {
		if (tag instanceof CollectionTag collectionTag)
			...
			new HashSet<>(compoundTag.keySet()).forEach(compoundTag::remove);
	}
